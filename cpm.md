---
allowed-tools: Bash(git add:*), Bash(git status:*), Bash(git diff:*), Bash(git commit:*), Bash(git push:*), Bash(git log:*), Bash(git checkout:*), Bash(git merge:*), Bash(git init:*), Bash(git branch:*), Bash(git rev-parse:*), Bash(git remote:*), Bash(git config:*), Read, Edit, Glob
...
- **If not a git repo**: tell the user, then run `git init -b main` to initialize a local repo. Continue with the commit flow but skip the push step (there's no remote yet). Skip the merge step since `main` is already the current branch.
...
### Step 6 — Merge with main

If the current branch is not `main`:
1. Switch to main: `git checkout main`
2. Merge the original branch: `git merge <original-branch>`
3. If a remote exists, push main: `git push`
4. Switch back to the original branch: `git checkout <original-branch>`

If the current branch is already `main`, skip this step.
